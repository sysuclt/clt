...
Tokamak::Tokamak() {//后面记得加入许多接口配置
	...
	if (rank == 0) {
		for (int i = 0; i < 10; i++) {
			toco_gather[i] = new float**[t_size];
			for (int j = 0; j < t_size; j++) {
				toco_gather[i][j] = new float*[r_size];
		...
		toco_t		= toco_gather[8];
		toco_r		= toco_gather[9];
	}
	else
		for (int i = 0; i < 10; i++)
			toco_gather[i] = NULL;
...
void Tokamak::readin() {
	...
	char *data_file_name[] = {"Bt_nova.dat", "Br_nova.dat", "Bp_nova.dat", \
							  "vt_nova.dat", "vr_nova.dat", "vp_nova.dat", \
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < p_size; j++) {
			...
			for (int k = 0; k < t_size; k++)
				for (int u = 0; u < r_size; u++) {
					x[k*t_size+u] = toco_t[k][u][j];/*顺序可能有问题！！！*/
					y[k*t_size+u] = toco_r[k][u][j];/*顺序可能有问题！！！*/
