...
!mpi   ----------------------------------------------------------------
    ...
    if(dt.lt.1.e-5) goto 900 !时间步长太小则跳出主程序循环
...
!!boundary --->
    ...
    lbndxfix(2)=.false. !p
    lbndxfix(3)=.true.  !vr 自由边界
    lbndxfix(4)=.false. !vy
    lbndxfix(5)=.false. !vp
    lbndxfix(6)=.true.  !br 自由边界
    lbndxfix(7)=.false. !by
    lbndxfix(8)=.false. !bp
...
!ws**************************************************************
subroutine map_nova
! 把从nova读出来的数据插值到我们现在的坐标，因为nova的网格分割和我们案例中的case不一样
    USE DECLARE
...
! --------------------- 非规则网格插值 ------------------------
    ...
    do jx=1,mxt
        do jz=1,mzt
            ...
            tpt(jx,jz)=atan2(pst_dz(jx,jz),pst_dx(jx,jz)) !垂直磁面方向，与tht相对应
...
! mpsa=npsi=111
    ...
    do jz=iz_first,iz_last
        do jx=ix_first,ix_last
            psi(jx,jz)   =   pst(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2) ! 单个cpu环坐标半径 pst是全局环坐标半径
            psi_dx(jx,jz)=pst_dx(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2)
            psi_dz(jx,jz)=pst_dz(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2) ! psi_dx,psi_dz 只是物理上有意义，不用管他。
            rr2(jx,jz)   =  rr2t(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2)
            rr(jx,jz)    =   rrt(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2)
            thxz(jx,jz)  =   tht(nrkx(nrank)*mxm+jx-2,nrkz(nrank)*mzm+jz-2) ! 切线方向theta total
...
subroutine valb8_atlastgrid(f8xz)
    ...
    integer is
    dimension f8xz(mx,mz,my,8),fst(n2th+5,mps4:mps,my,8),f1s(mbm_nrk,mps4:mps),fsxz(mbm_nrk,8)
    ! n2th=2*(nthe-1)=2*(101-1)=200, mpsa=npsi=111, mps4=mpsa-nda=111-4*ids=111-4*1=107, npsip=npsi+1=111+1=112, mbm_nrk=4*mx+4*mz, mx=mxt/nprx+4
    include 'mpif.h'
