...
PetscScalar x_start = 0, x_end = pi;
//x的取值范围
...
int main(int argc, char* argv[]) {
	...
	for (PetscInt i = 0; i < x_size; i++)
		for (PetscInt j = 0; j < y_size; j++) {
			if (isBoundary(i, j)) {
				if (i == 0) {
					##LEFT_SIDE_BORDER{position2row(i,j)}
				}
				else if (i == pi) {
					##RIGHT_SIDE_BORDER{position2row(i,j)}
				}
				else if (j == 0) {
					##DOWN_SIDE_BORDER{position2row(i,j)}
				}
				else if (j == 1) {
					##UP_SIDE_BORDER{position2row(i,j)}
				}
			}
			else {
				##FIVE_POINT_DIFFERENCE_FORMAT_EQUATION{position2row(i,j)}
			}
		}
	A.startAsm();
	b.startAsm();
